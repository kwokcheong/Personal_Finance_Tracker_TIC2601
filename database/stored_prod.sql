...
BEGIN
    ...
    IF MONTH_DIFF = 0 THEN 
		(SELECT E.category, SUM(E.amount) FROM expenses E WHERE E.userID = USER_ID AND DATE(E.created_at) BETWEEN EARLIEST_DATED_RECORD AND LATEST_DATED_RECORD GROUP BY E.category ORDER BY E.category ASC);
	ELSE  
		(SELECT E.category, (SUM(E.amount)/ MONTH_DIFF) FROM expenses E WHERE E.userID = USER_ID AND DATE(E.created_at) BETWEEN EARLIEST_DATED_RECORD AND LATEST_DATED_RECORD GROUP BY E.category ORDER BY E.category ASC);
	END IF; 
END$$
DELIMITER;
...
BEGIN
    ...
    IF MONTH_DIFF = 0 THEN 
		(SELECT I.category, SUM(I.amount) FROM incomes I WHERE I.userID = USER_ID AND DATE(I.created_at) BETWEEN EARLIEST_DATED_RECORD AND LATEST_DATED_RECORD GROUP BY I.category ORDER BY I.category ASC);
	ELSE  
		(SELECT I.category, (SUM(I.amount)/ MONTH_DIFF) FROM incomes I WHERE I.userID = USER_ID AND DATE(I.created_at) BETWEEN EARLIEST_DATED_RECORD AND LATEST_DATED_RECORD GROUP BY I.category ORDER BY I.category ASC);
	END IF; 
END$$
DELIMITER;