...
BEGIN
    ...
    IF MONTH_DIFF = 0 THEN 
		SET FINAL_OUTPUT = (SELECT SUM(E.amount) FROM expenses E WHERE E.userID = ip_user AND DATE(E.created_at) BETWEEN EARLIEST_DATED_RECORD AND LATEST_DATED_RECORD);
	ELSE 
        SET FINAL_OUTPUT = (SELECT (SUM(E.amount)/ MONTH_DIFF) FROM expenses E WHERE E.userID = ip_user AND DATE(E.created_at) BETWEEN EARLIEST_DATED_RECORD AND LATEST_DATED_RECORD);
	END IF; 
RETURN FINAL_OUTPUT;
END$$
DELIMITER;
...
BEGIN
	...
	IF MONTH_DIFF = 0 THEN 
		SET FINAL_OUTPUT = (SELECT SUM(I.amount) FROM incomes I WHERE I.userID = ip_user AND DATE(I.created_at) BETWEEN EARLIEST_DATED_RECORD AND LATEST_DATED_RECORD);
	ELSE 
        SET FINAL_OUTPUT = (SELECT (SUM(I.amount)/ MONTH_DIFF) FROM incomes I WHERE I.userID = ip_user AND DATE(I.created_at) BETWEEN EARLIEST_DATED_RECORD AND LATEST_DATED_RECORD);
	END IF; 
RETURN FINAL_OUTPUT;
END$$
DELIMITER;
